...
async def cb_convert_points(callback: CallbackQuery, session: AsyncSession, db_user: User):
    ...
    text = (
        ...
        f"Mínimo: <b>{min_pts}</b>\n"
        f"Multiplicador: <b>{mult}</b>\n\n"
        f"Ex: {min_pts} × {mult} = R$ {float(min_pts) * float(mult or 0):.2f}"
    )
    b = InlineKeyboardBuilder()
    if db_user.affiliate_points >= min_pts and min_pts > 0:
        b.row(
            InlineKeyboardButton(
                text="✅ Converter agora", callback_data="affiliate_do_convert"
            )
        )
...
async def cb_do_convert(callback: CallbackQuery, session: AsyncSession, db_user: User):
    try:
        amount = await AffiliateService.convert_points_to_balance(session, db_user.id)
