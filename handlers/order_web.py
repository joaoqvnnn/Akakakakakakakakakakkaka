...
async def page_pedido(request: web.Request) -> web.Response:
    ...
    path = Path(__file__).resolve().parent.parent / "web" / "pedido.html"
    if not path.exists():
        return web.Response(text="pedido.html não encontrado", status=404)
...
async def api_pedido_meta(request: web.Request) -> web.Response:
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not order:
            return web.json_response({"ok": False, "error": "Link inválido"}, status=403)
        ...
        return web.json_response(
            {
                ...
                "expires": order.expires_at.strftime("%d/%m/%Y")
                if order.expires_at
                else "—",
...
async def api_pedido_unlock(request: web.Request) -> web.Response:
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not user or not OrderSecureService.check_user_password(
            user, password, fallback
        ):
            return web.json_response(
                {"ok": False, "error": "Senha incorreta"}, status=401
            )

        product = await session.get(Product, order.product_id)
        store = await SettingsService.get(
            session, "store_name", settings.STORE_NAME
        )
        activation = (
        ...
        pdf = build_order_pdf(
            ...
            expires_str=order.expires_at.strftime("%d/%m/%Y")
            if order.expires_at
            else "—",
            payment_method=order.payment_method.value,
